...
#
#   reject   🚫   permanently off-limits; --bypass cannot override
#   guarded  🔒   requires --bypass; without it: rejected + logged
...
#   • Vault uses field-level merge: only the keys you write
...
#
# Note: The deprecated fields confirm_double, confirm, and warn are
# still accepted for backward compatibility. Their values are merged
# into guarded at load time. For new config files, use 'guarded'
# directly.
...
guarded = [
  # Home directory app data - requires --bypass
  "~/.config/**",
  "~/.local/share/**",
  # Documents directory - requires --bypass
  "~/Documents/**",
  # Desktop files - requires --bypass
  "~/Desktop/**",
  # Downloads directory - requires --bypass
  "~/Downloads/**",
]

# Command-level overrides - apply stricter protection for specific commands
# Example: global rules set ~/.config/** to guarded,
