...
// WrapperSentinel is a marker line embedded in every wrapper script
// generated by `cmdguard init`. findRealCommand uses it to recognise
// cmdguard wrapper scripts on PATH and skip them, even when the
// wrapper points to a *different* cmdguard binary than the one
// currently running (e.g. user has cmdguard installed system-wide
// while a sandboxed CMDGUARD_CONFIG_DIR is in use, or two cmdguard
// installs coexist).
//
// Without this guard, findRealCommand would resolve `rm` to the
// wrapper, exec it, and recurse. Path-prefix-based skip
// (config.ConfigDir()/bin) does not cover wrappers in *other*
// cmdguard installs because their bin dir lives elsewhere.
//
// Format is versioned (v1) so the detector can evolve without
// breaking existing wrappers; mismatched versions are still
// recognised as wrappers — only the version suffix can change.
const WrapperSentinel = "# cmdguard:wrapper:v1"

// WrapperSentinelPrefix is the version-agnostic prefix used for
// detection. Any future v2/v3 wrapper still gets recognised because
// the detector matches on this prefix, not the full string.
const WrapperSentinelPrefix = "# cmdguard:wrapper:"
